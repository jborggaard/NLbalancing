...
%
%  This script generates the tables found in Section V.A. of the paper
%    Nonlinear balanced truncation:  Part 1--Computing energy functions, 
%    by Kramer, Gugercin, and Borggaard, arXiv.
%
...
for n=[8,16,32,64]
  ...
  wzInit = kronPolyEval(w,zInit,degree);
  fprintf('%12.6e \\\\ \n',wzInit)
end
...
for degree=[2,3,4,5,6]
  ...
  [v] = approxPastEnergy(full(A),N,B,C,eta,degree,true);
  for d=2:degree, v{d} = v{d}.'; end
  vzInit = kronPolyEval(v,zInit,degree);
  fprintf('%12.6e & ',vzInit)

  [w] = approxFutureEnergy(full(A),N,B,C,eta,degree,true);
  for d=2:degree, w{d} = w{d}.'; end
  wzInit = kronPolyEval(w,zInit,degree);
  fprintf('%12.6e \\\\ \n',wzInit)
end
